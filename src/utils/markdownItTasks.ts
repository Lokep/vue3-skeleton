...
export default function (md: MarkdownIt, options: IOption = {}) {
  if (options) {
...
function todoify(token: Token, TokenConstructor: any) {
  ...
  if (useLabelWrapper && token.children) {
    if (useLabelAfter) {
      token.children.pop();
...
function afterLabel(content: string, id: string, TokenConstructor: any) {
  var token = new TokenConstructor('html_inline', '', 0);
